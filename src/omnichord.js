module.exports = function Omnichord(Midiboard){
  ...
  oc.mapInputToKey = function(x, midiMsg) {
    ...
    if(whichRow !== null && whichNote !== null && whichRow !== 'strum') {
      oc.clearLastChord();
  ...
  oc.selectChord = function(whichScale, whichNote, buttonInt){
    ...
    oc.mb.outgoing(144, oc.meta.currentChord.buttonInt, oc.meta.colors[oc.meta.currentChord.scheme].on);
  };

  oc.showOnlyScale = function(scale) {
  ...
  oc.clearPlayingNotes = function(){
    console.log("CURRENTLY PLAYING", oc.currentlyPlayingNotes);
    for(let note in oc.currentlyPlayingNotes) {

      note = oc.currentlyPlayingNotes[note];
      console.log("ATTEMPTING TO CLEAR NOTE", note);
      oc.playMidiNote(128, note);
    }
    if(oc.currentlyPlayingNotes.length !== 0) {
      oc.clearPlayingNotes();
    }
  };
